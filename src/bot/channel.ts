...
export async function startChannel(deps: StartChannelDeps): Promise<BridgeChannel> {
  ...
  const opts: LarkChannelOptions = {
    ...
    outbound: {
      // FIX(fork): 600ms between stream card updates — the cardkit
      // element-update API is rate-limited, and long replies hammer it.
      // 400ms caused frequent update failures mid-stream on long outputs.
      streamThrottleMs: 600,
    },
  ...
  return {
    ...
    disconnect: async () => {
      ...
      pending.cancelAll();
      // FIX(fork): let in-flight runs finish instead of killing them. A long
      // reply that is still streaming must not be aborted just because the
      // WS hiccuped and a reconnect/restart was triggered (that was the
      // "long replies die" bug). While we wait, the old channel is still
      // alive (restart is connect-before-disconnect), so final replies still
      // go out. Only force-stop after the grace window.
      const runGraceMs = getDisconnectRunGraceMs(controls.cfg);
      const allIdle = await activeRuns.waitForIdle(runGraceMs);
      if (!allIdle) {
        log.warn('disconnect', 'run-grace-expired', {
          graceMs: runGraceMs,
...
/**
 * How long a disconnect waits for in-flight runs to finish before force-
 * stopping them. Preferences override; default 90s. 0 disables the wait
 * (immediate stop, the pre-fork behavior).
 */
function getDisconnectRunGraceMs(cfg: AppConfig): number {
  const raw = cfg.preferences?.disconnectRunGraceMs;
  if (typeof raw !== 'number' || !Number.isFinite(raw)) return 90_000;
  return Math.min(600_000, Math.max(0, Math.floor(raw)));
}
...
async function runAgentBatch(deps: RunBatchDeps): Promise<void> {
  ...
  //
  // CoT mode skips it entirely: the thinking-process bubble (message_cot) is
  // created immediately and is feedback enough — the placeholder would only
  // sit on top of it as noise ("正在思考…" + the bubble).
  const cotMode = getCotMessages(controls.cfg) !== 'off';
  const ackPlaceholder = cotMode ? undefined : postAckPlaceholder(channel, chatId, sendOpts);
  ...
  try {
    ...
    if (replyMode === 'card') {
      ...
      try {
        ...
        // Stream failure must not skip the dedicated final reply (claude/mimo
        // fall back to it; codex logs and continues too). Rethrowing here would
        // drop the conclusion for claude/mimo.
      ...
      // failed. codex holds it in final_text (finalReplyState); claude/mimo
      // fall back to the full state as a standalone card, but only when the
      // stream actually opened and wasn't abandoned (else fallback sent it).
      ...
      try {
        ...
        if (controls.profileConfig.agentKind === 'codex') {
          ...
          log.warn('stream', 'final-reply-after-stream-error', { mode: replyMode });
        }
      }
      await recallIfEmptyStreamedReply(channel, progress, filterForPrefs(latestState), scope);
