...
async function handleRuntimeLockConflict(
  ...
  // FIX(fork): the holder process is already dead (crash, zombie, or a
  // daemon restart loop that left a stale registry entry / lock behind).
  // Clean up automatically and retry instead of failing — this was the
  // repeated "已有 bridge 进程占用" restart loop in non-interactive mode.
  if (!isAlive(err.meta.pid)) {
    console.warn(
      `  holder pid ${err.meta.pid} 已不存在，自动清理陈旧锁后重试…`,
    );
    await cleanStaleRuntimeLock(err.target).catch((e) =>
      console.warn(`  stale lock cleanup failed: ${(e as Error).message}`),
    );
    return 'retry';
  }
...
/**
 * Remove a runtime lock file and its meta file after confirming the holder
 * is dead. The lock file itself is a proper-lockfile lock (no live updater),
 * and the meta file pins the dead pid — both must go or the next acquire
 * keeps reporting a conflict (or waits for the 30s stale window).
 */
async function cleanStaleRuntimeLock(target: string): Promise<void> {
  await Promise.allSettled([
    unlink(target).catch(() => undefined),
    unlink(runtimeLockMetaFile(target)).catch(() => undefined),
  ]);
}
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error(
      `当前 ${err.kind === 'profile' ? 'profile' : 'app'} 已有 bridge 进程占用；` +
        '非交互模式无法确认停止，请先用 `feishu-agent-bridge ps` 查看并用 `feishu-agent-bridge kill <bot id>` 停止后重试',
    );
  }
