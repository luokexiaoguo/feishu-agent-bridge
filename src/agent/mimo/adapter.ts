...
export interface MimoAdapterOptions {
   ...
   * letting the writer's partial state be rebuilt next time.
   *
   * Long replies legitimately pause mid-generation (thinking, a long tool
   * call, a big buffered block), so this must be generous — a too-small
   * value kills long replies mid-flight, which is exactly the failure mode
   * this fork fixes. Default 180s; `idleSeconds: 0` in the profile disables
   * it entirely (process exits on its own or via the run stop path).
   */
...
export class MimoAdapter implements AgentAdapter {
  ...
  constructor(opts: MimoAdapterOptions = {}) {
    ...
    // 0 disables the idle-finish heuristic entirely.
    this.idleTimeoutMs = opts.idleTimeoutMs === 0 ? 0 : (opts.idleTimeoutMs ?? 180_000);
  }
...
async function* createEventStream(
  ...
  // idleTimeoutMs === 0 disables this entirely (the run then ends via the
  // normal stop path when the turn is over).
