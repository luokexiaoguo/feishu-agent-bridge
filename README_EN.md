...
`/compact` is a **pure passthrough**: the bridge only mediates Feishu ↔ CLI, and each agent's own compaction command does the real work — no summarizer config needed:

- **claude**: `claude -p --resume <session> "/compact [focus]"` (headless accepts the slash command)
- **mimo**: `mimo run --session <id> "/compact"`
- **openclaw**: `openclaw sessions compact <key>`
- opencode / codex / hermes headless CLIs have no compaction command and are not supported

```jsonc
{
  "profiles": {
    "claude": {
      "compaction": { "enabled": true }   // master switch, default true; may be omitted
    }
  }
}
```
...
| --- | --- |
| `enabled` | `/compact` master switch, default `true` |

> Legacy summarizer config (`keepRounds` / `llm`) was removed with the native-passthrough rework; leftover keys in config are ignored.
...
- `/compact [focus]` — **Context compression (native passthrough)**: claude runs its own `/compact` (equivalent to `claude -p --resume "/compact"` in a terminal; the argument is a focus instruction); mimo runs `mimo run --session <id> "/compact"`; openclaw runs `opencode sessions compact` → `openclaw sessions compact`. opencode / codex / hermes headless CLIs have no compaction command and are not supported
